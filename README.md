...
Battery-powered BLE remote mechanical switch: C state machine and storage logic on Zephyr (BLE, motor, NVS, button, ADC). Target board nRF5340 DK application core (or use another board overlay).
...
2. Build the Zephyr application (from project root):
   ```bash
   ./scripts/build.sh
   ```
...
- **`src/`** — C: main, application state machine, BLE GATT service, hw_glue (motor/LED/NVS/battery), button, timer_glue.
